...
from steg_hider import hide_message, generate_keys
...
def create_hunt_chain(num_clues, base_images_dir, output_dir, password=None, public_key=None, theme="vault", players=20, factions=4, difficulty_start=1):
    ...
    base_images = list(Path(base_images_dir).glob("*.png")) + list(Path(base_images_dir).glob("*.jpg"))
    if not base_images:
    ...
    for i in range(num_clues):
        ...
        try:
            result = hide_message(
                ...
                auto_tune=True,
                expected_corruption=10  # Expect some compression
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_clues", type=int, default=5, help="Number of clues in the chain")
    parser.add_argument("--base_images", default="images", help="Directory with base images")
