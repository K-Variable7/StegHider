...
def test_chunk_embed_reassemble_rsa(tmp_path):
    ...
    ok = chunk_and_embed_file(
        str(infile), [str(cover)], str(out_dir), public_key_path=str(pub)
    )
    ...
    ok2 = reassemble_from_images(
        stego_images, str(out_reassembled), private_key_path=str(priv)
    )
...
def test_auto_tune_nsym_for_corruption(tmp_path):
    ...
    for pct in [5, 10, 15, 20, 30]:
        ...
        ok2 = reassemble_from_images(
            [str(zeroed)], str(out_reassembled), private_key_path=str(priv)
        )
...
def test_recovery_with_corruption(tmp_path):
    ...
    ok2 = reassemble_from_images(
        [str(corrupted)], str(out_reassembled), private_key_path=str(priv)
    )
